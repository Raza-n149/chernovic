...
window.onload = function () {
    ...
    function initReviews() {
        ...
        // Обработчики кнопок
        $('.review-prev').on('click', showPrevReview);
        $('.review-next').on('click', showNextReview);
    }
    
    function showPrevReview() {
        reviews.eq(currentReview).removeClass('active').fadeOut(300);
        ...
        setTimeout(function() {
            reviews.eq(currentReview).addClass('active').fadeIn(300);
            updateReviewCounter();
            updateContainerHeight();
        }, 300);
    }
    ...
    function updateReviewCounter() {
        $('.current-review').text((currentReview + 1).toString().padStart(2, '0'));
    }
    
    function updateContainerHeight() {
        const currentHeight = reviews.eq(currentReview).outerHeight();
        $('.review-container').css('min-height', currentHeight + 'px');
    }
    
    // Инициализируем отзывы
    initReviews();
